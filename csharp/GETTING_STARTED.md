# Getting Started with Pascal Triangle kata in C#
...
> ***Reminder***: the commands below should be run from [Kata-PascalTriangle/csharp]() directory
...
> ***Reminder***: the command below should be run from [Kata-PascalTriangle/csharp]() directory
...
`Kata-PascalTriangle` / `csharp` / `Kata-PascalTriangle.sln`

After loading the solution:

1. Make sure the 'PascalTriangle.Test' project is set as the Start-up Project.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-PascalTriangle` / `csharp`
