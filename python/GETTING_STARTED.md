# Getting Started with Pascal Triangle kata in Python
...
> ***Reminder***: the commands below should be run from [Kata-PascalTriangle/python](.) directory
...
`File` > `Open` > `Kata-PascalTriangle` > `python`
...
> ***Reminder***: the command below should be run from [Kata-PascalTriangle/python](.) directory
...
Choose `Open Folder`, navigate to `Kata-PascalTriangle` / `python` then click `Select Folder`.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-PascalTriangle` / `python`
